...
from app.infra.stream_bridge import MemoryStreamBridge, build_stream_bridge
...
async def test_publish_subscribe(bridge: MemoryStreamBridge):
    """Three events followed by a terminal end event should be received in order."""
    run_id = "run-1"
    ...
    assert received[3].event == "end"
    assert received[3].data is None
...
async def test_heartbeat(bridge: MemoryStreamBridge):
    ...
    await bridge._get_or_create_stream(run_id)  # ensure stream exists
...
async def test_history_is_bounded():
    """Retained history should be bounded by queue_maxsize plus terminal event."""
...
async def test_publish_end_terminates_even_when_history_is_full():
    """publish_end() should terminate subscribers and append a terminal event."""
    bridge = MemoryStreamBridge(queue_maxsize=2)
    ...
    await bridge.publish_end(run_id)
    assert [entry.event for entry in stream.events] == ["event-1", "event-2", "end"]
...
async def test_concurrent_tasks_end_sentinel():
    ...
    for run_id in run_ids:
        ...
        assert events[-1].event == "end", f"Run {run_id} did not receive end event"
...
async def test_make_stream_bridge_defaults():
    """build_stream_bridge() with no config yields a MemoryStreamBridge."""
    async with build_stream_bridge() as bridge:
